...
#define SPI_CS_PIN   10   // Pin Chip Select (CS)

//Adresy Rejestrow
#define PWR_CONF_REG 0x7C //Rejestr PWR_CTRL
#define INIT_CTRL_REG 0x59 //Rejestr INIT_CTRL
#define INIT_DATA_REG 0x53 //Rejestr INIT_DATA
#define INTERNAL_STATUS_REG 0x21 //Rejestr INTERNAL_STATUS
#define PWR_CTRL_REG 0x7D //Rejestr PWR_CTRL
#define ACC_CONF_REG 0x40 //Rejestr ACC_CONF_REG
#define ACC_RANGE_REG 0x41 //Rejestr ACC_RANGE
#define GYR_CONF_REG 0x42 //Rejestr GYR_CONF
#define GYR_RANGE_REG 0x43 //Rejestr GYR_RANGE_REG


#define GRAVITY_ACC 9.8115 //Przyspieszenie ziemskie we Wroclawiu w jednostkach m/s2
#define GYRO_SCALE 16.384 // Skala dla zakresu zyro +- 2000 dps ZMIENIC PRZY ZMIANIE ZAKRESU
...
esp_err_t bmi270_write_config_file(uint8_t reg, const uint8_t *array, size_t size) {
    esp_err_t ret;
...
void init_spi() {
    spi_bus_config_t buscfg = {
        ...
        .max_transfer_sz = 8200 // Maksymalny rozmiar transferu (w bajtach)
    };

    spi_device_interface_config_t devcfg = {
        .clock_speed_hz = 1000000,         // Prędkość zegara SPI: 1 MHz
...
esp_err_t bmi280_init(const uint8_t *config, size_t length) {
    ...
    bmi270_write_config_file(INIT_DATA_REG, bmi270_config_file, sizeof(bmi270_config_file));
...
esp_err_t bmi280_read_data()
{
    ...
    float scale_factor = GRAVITY_ACC / 4096;
    float acc_x = acc_x_raw * scale_factor;
    float acc_y = acc_y_raw * scale_factor;
    float acc_z = acc_z_raw * scale_factor;
    ...
    float scale_factor_gyro = 1.0 / GYRO_SCALE; // Skalowanie dla ±2000 dps
    float gx = raw_gx * scale_factor_gyro;
    float gy = raw_gy * scale_factor_gyro;
    float gz = raw_gz * scale_factor_gyro;

    ESP_LOGI(TAG, "Przyspieszenie (m/s^2): a_x=%.2f, a_y=%.2f, a_z=%.2f\n", acc_x, acc_y, acc_z);
    ESP_LOGI(TAG, "Predkosci katowe (dps): omega_x=%.2f, omega_y=%.2f, omega_z=%.2f\n", gx, gy, gz);
